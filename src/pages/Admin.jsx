...
export default function Admin() {
  ...
  async function fetchRoster(headersOverride) {
    const res = await fetch("/api/roster", { headers: headersOverride || authedHeaders });
    const data = await safeJsonOrText(res);
  ...
  async function fetchFinalStatus(headersOverride) {
    const res = await fetch("/api/admin/final-status", { headers: headersOverride || authedHeaders });
    const data = await safeJsonOrText(res);
  ...
  async function refreshAll(keyOverride) {
    ...
    try {
      ...
      await Promise.all([fetchRoster(headers), fetchInbox(headers), fetchFinalStatus(headers)]);
    } catch (e) {
      setErr(e?.message || String(e));
    } finally {
      setLoading(false);
  ...
  async function downloadFinal(playerId) {
    ...
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      // Server sets content-disposition filename, but iOS sometimes ignores it; keep a default
    ...
    <div style={{ maxWidth: 980, margin: "0 auto" }}>
      ...
      <div className="card">
        ...
        <div style={{ marginTop: 10, display: "grid", gap: 10 }}>
          {roster.length === 0 ? (
            <div style={{ opacity: 0.75 }}>No roster found yet.</div>
          ) : (
            roster.map((p) => {
              const pid = p.id;
