...
const EMPTY_ADMIN = { imports: [], funds: [], commitments: [], mappings: [], audit: [], documents: [], forecasts: [], boardMembers: [], pinStorageReady: true };
  ...
  return (
    <div>
      <div className="finance-chart is-balance" role="img" aria-label={`Official and calculated historical balances from ${visibleRange}`}>
        <svg viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="xMidYMid meet">
          {areaSegments.filter((segment) => segment.length > 1).map((segment, index) => <polygon key={index} points={`${segment[0].x},${y(0)} ${segment.map((point) => `${point.x},${point.y}`).join(" ")} ${segment.at(-1).x},${y(0)}`} className="finance-balance-chart-area" />)}
...
function Overview({ dashboard }) {
  ...
  const balanceStatus = !overview.hasReconciledBalances
    ...
    : overview.balanceIsPreliminary
      ...
      : { label: "Reconciled balance", tone: "is-good" };
  return (
...
function Transactions({ rows, filters, setFilters, bootstrap, isEditor, onEdit, onExport, loading }) {
  const hasPreliminaryRows = rows.some((transaction) => transaction.periodStatus !== "published");
  return <div className="finance-section-stack"><section className="card finance-panel"><div className="finance-section-heading"><div><h2>Transactions</h2><p>Internal transfers remain visible but are excluded from income and expense totals.</p></div><button className="btn-secondary" onClick={onExport}>Export filtered CSV</button></div><div className="finance-filter-grid"><input className="input" type="search" placeholder="Search description or notes" value={filters.search} onChange={(event) => setFilters((current) => ({ ...current, search: event.target.value }))} /><input className="input" type="month" value={filters.month} onChange={(event) => setFilters((current) => ({ ...current, month: event.target.value }))} /><select className="input" value={filters.category} onChange={(event) => setFilters((current) => ({ ...current, category: event.target.value }))}><option value="">All categories</option>{bootstrap.categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select><select className="input" value={filters.classification} onChange={(event) => setFilters((current) => ({ ...current, classification: event.target.value }))}><option value="">All types</option><option value="income">Income</option><option value="expense">Expense</option><option value="transfer">Transfer</option></select></div></section>{hasPreliminaryRows ? <div className="finance-alert is-warning"><strong>Preliminary transactions.</strong> These imported rows are visible before statement validation, reconciliation, or publication.</div> : null}{loading ? <LoadingCard text="Loading transactions…" /> : rows.length ? <div className="finance-transaction-list">{rows.map((transaction) => <article className="card finance-transaction-card" key={transaction.id}><div className="finance-transaction-date">{transaction.transactionDate}</div><div className="finance-transaction-summary"><h3>{transaction.description}</h3><div className="finance-transaction-support"><div className="finance-chip-row"><span>{transaction.classification}</span><span>{transaction.categoryName}</span>{transaction.accountName && transaction.accountName !== "Consolidated historical source" ? <span>{transaction.accountName}</span> : null}{transaction.isInternalTransfer ? <span>Internal transfer</span> : null}<span>{transaction.reconciliationStatus}</span><span>{transaction.periodStatus === "published" ? "Published" : "Preliminary"}</span></div><div className="finance-transaction-meta">Source: {transaction.sourceFilename || "Manual"}{transaction.sourceRow ? `, row ${transaction.sourceRow}` : ""} · {transaction.statementMonth}</div></div></div><strong className={`finance-transaction-amount ${transaction.amountCents < 0 ? "is-negative" : "is-positive"}`}>{money(transaction.amountCents)}</strong>{isEditor ? <button className="btn-secondary btn-sm" onClick={() => onEdit(transaction)}>Edit</button> : null}</article>)}</div> : <EmptyState>No transactions match these filters.</EmptyState>}</div>;
}
...
function BoardMemberAdmin({ members, pinStorageReady, onMutate, busy }) {
  ...
  return (
    <section className="card finance-panel">
      ...
      <div className={`finance-alert ${pinStorageReady ? "is-info" : "is-danger"}`}>{pinStorageReady ? "Current PINs are visible here so an admin can remind a Board member without resetting their login." : "Board PIN display setup is incomplete. Apply D1 migration 0012 before adding or updating PINs."}</div>
      <form className="finance-member-add" onSubmit={addMember}>
        ...
        <label><span>Six-digit PIN</span><input className="input" type="text" inputMode="numeric" maxLength="6" autoComplete="off" placeholder="6-digit PIN" value={pin} disabled={!pinStorageReady} onChange={(event) => setPin(cleanPin(event.target.value))} /></label>
        <button className="btn" disabled={busy || !pinStorageReady || name.trim().length < 2 || pin.length !== 6}>Add Board member</button>
      </form>
      <div className="finance-member-list">
        {members.length ? members.map((member) => {
          const nextPin = replacementPins[member.id] || "";
          return <article className={`finance-member-row ${member.isActive ? "" : "is-inactive"}`} key={member.id}><div><strong>{member.name}</strong><small>{member.isActive ? "Active" : "Removed"}{member.lastLoginAt ? ` · Last signed in ${new Date(member.lastLoginAt).toLocaleString()}` : " · Never signed in"}</small></div><div className="finance-member-pin"><small>Current PIN</small><strong>{member.pin || (pinStorageReady ? "Set once to make visible" : "Migration required")}</strong></div><input className="input" aria-label={`New PIN for ${member.name}`} type="text" inputMode="numeric" maxLength="6" autoComplete="off" placeholder="New 6-digit PIN" value={nextPin} disabled={!pinStorageReady} onChange={(event) => setReplacementPins((current) => ({ ...current, [member.id]: cleanPin(event.target.value) }))} /><button className="btn-secondary btn-sm" disabled={busy || !pinStorageReady || nextPin.length !== 6} onClick={() => savePin(member)}>{member.isActive ? "Update PIN" : "Restore with PIN"}</button>{member.isActive ? <button className="btn-danger btn-sm" disabled={busy} onClick={() => removeMember(member)}>Remove</button> : null}</article>;
